...
└── sql/
    ...
    └── 09_seed_admin_user.sql     # test users + Java BCrypt hashes (see Postman seedPassword)
```
...
**Override DB connection as needed:**
...
Seeded app passwords live in `09_seed_admin_user.sql` as BCrypt strings from Java. Set Postman **`seedPassword`** to the **plaintext** you used when generating those hashes.

---

## Seeded users (`09_seed_admin_user.sql`)

On first run (and on each re-run, hashes are refreshed via `UPDATE`):
...
- **Password:** the plaintext you used in Java to generate the BCrypt hashes in that file (often one password for every user). Put the same value in Postman **`seedPassword`**.
- Login accepts **username** or **email**, case-insensitively.
...
- **BCrypt in seeds.** `09_seed_admin_user.sql` stores Java/Spring-compatible `$2a$` hashes; verify with `PasswordEncoder.matches(plain, hash)`.
